...
class StructuredMockLLMClient extends LLMClient:
  ...
  def complete(conversation: Conversation, options: CompletionOptions): Either[org.llm4s.error.LLMError, Completion] =
    val lastUserMessage: String = conversation.messages
      .collect {
    ...
    val lastSystemPrompt: String =
      conversation.messages.collect { case msg: SystemMessage => msg.content }.lastOption.getOrElse("")
  ...
  private def generateResponse(userMessage: String, systemPrompt: String): String =
    ...
    if containsSAPExample(userMessage, systemPrompt) then generateSAPErrorResponse(userMessage, systemPrompt)
    else if promptType == "classifier" then generateClassificationResponse(userMessage, systemPrompt)
    else if promptType == "planner" then generatePlanResponse(userMessage, systemPrompt)
    else if promptType == "extraction" then generateExtractionResponse(userMessage, systemPrompt)
    else if promptType == "multiagent" then generateMultiAgentResponse(userMessage, systemPrompt)
    else if promptType == "chain" then generateChainResponse(userMessage, systemPrompt)
    else if promptType == "workflow" then generateWorkflowResponse(userMessage, systemPrompt)
    else if promptType == "toolcall" then generateToolCallResponse(userMessage, systemPrompt)
    else generateGenericResponse(userMessage)

  private def detectPromptType(systemPrompt: String): String =
    if systemPrompt.contains("classif") || systemPrompt.contains("classifier") then "classifier"
    else if systemPrompt.contains("plan") || systemPrompt.contains("extract plan") then "planner"
    else if systemPrompt.contains("extract") || systemPrompt.contains("parse list") then "extraction"
    else if systemPrompt.contains("host") || systemPrompt.contains("specialist") then "multiagent"
    else if systemPrompt.contains("chain") || systemPrompt.contains("intermediate") then "chain"
    else if systemPrompt.contains("workflow") || systemPrompt.contains("graph") || systemPrompt.contains("async") then
      "workflow"
    else if systemPrompt.contains("ReAct") || systemPrompt.contains("tool") then "toolcall"
    else "generic"
  ...
  private def generateSAPErrorResponse(userMessage: String, systemPrompt: String): String =
    if userMessage.contains("trailing comma") then """{"category": "math", "confidence": 0.95,}"""
    else if userMessage.contains("single quote") then """{'category': 'science', 'confidence': 0.92}"""
    else if userMessage.contains("markdown fence") then """```json
{"category": "history", "confidence": 0.90}
```"""
    else """{"category": "other", "confidence": 0.88,"""
  ...
  private def generateClassificationResponse(userMessage: String, systemPrompt: String): String =
    if userMessage.contains("square root") || userMessage.contains("144") then
      ...
      """{"category": "history", "confidence": 0.90}"""
    else if userMessage.contains("weather") then """{"category": "other", "confidence": 0.88}"""
    else if userMessage.contains("customer") || userMessage.contains("support") then
      """{"role": "customer", "confidence": 0.91}"""
    else if userMessage.contains("manager") then """{"role": "manager", "confidence": 0.89}"""
    else if userMessage.contains("question") then """{"queryType": "question", "intent": "information seeking"}"""
    else if userMessage.contains("command") then """{"queryType": "command", "intent": "action request"}"""
    else if userMessage.contains("database") then """{"chainName": "database", "reason": "Requires data access"}"""
    else """{"category": "other", "confidence": 0.70}"""
  ...
  private def generatePlanResponse(userMessage: String, systemPrompt: String): String =
    ...
    else """{"steps": [{"index": 1, "description": "Analyze request"}], "totalDuration": 5}"""
  ...
  private def generateMultiAgentResponse(userMessage: String, systemPrompt: String): String =
    ...
    else """{"specialist": "general-agent", "rationale": "No specific domain identified"}"""
  ...
  private def generateChainResponse(userMessage: String, systemPrompt: String): String =
    ...
    else """{"result": "intermediate output", "status": "success"}"""
  ...
  private def generateWorkflowResponse(userMessage: String, systemPrompt: String): String =
    ...
    else """{"nodeType": "process", "action": "continue", "status": "pending"}"""
  ...
  private def generateToolCallResponse(userMessage: String, systemPrompt: String): String =
    ...
    else if userMessage.contains("search") then "I'll use the search tool to find relevant information."
    else if userMessage.contains("weather") then "Let me check the weather using the weather tool."
    else if userMessage.contains("book") || userMessage.contains("trip") then
      """I'll help you book your trip. Let me gather the details."""
    else "I'll help you with that task."
