...
object StepsExtractionStructuredExample extends IOApp.Simple:
  ...
  private def createLLMClient: IO[org.llm4s.llmconnect.LLMClient] =
    ExampleUtils.createLLMClient.recoverWith { case _: UnsupportedOperationException =>
      IO.pure(new StepsExtractionMockLLMClient())
    }
  ...
  def run: IO[Unit] =
    for
      ...
      recipe =
        "To make pancakes: 1. Mix flour, eggs, and milk (5 min). 2. Heat pan (2 min). 3. Pour batter and cook (3 min). 4. Flip and cook other side (3 min). 5. Serve (1 min)."
      ...
      setup =
        "Installation steps: 1. Download the installer. 2. Run the setup wizard. 3. Accept the license agreement. 4. Choose installation directory. 5. Click Install."
      ...
      workflow =
        "Deployment workflow: 1. Run tests (10 min). 2. Build Docker image. 3. Push to registry (5 min). 4. Update Kubernetes config. 5. Apply changes (2 min). 6. Verify deployment (3 min)."
