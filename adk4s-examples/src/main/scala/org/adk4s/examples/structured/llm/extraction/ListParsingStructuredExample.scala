...
object ListParsingStructuredExample extends IOApp.Simple:
  ...
  private def createLLMClient: IO[org.llm4s.llmconnect.LLMClient] =
    ExampleUtils.createLLMClient.recoverWith { case _: UnsupportedOperationException =>
      IO.pure(new ListParsingMockLLMClient())
    }
  ...
  def run: IO[Unit] =
    for
      ...
      _       <- result1.items.traverse_(item => IO.println(s"     ${item.index}. ${item.content}"))
      ...
      _       <- result2.items.traverse_(item => IO.println(s"     ${item.index}. ${item.content}"))
      ...
      _       <- result3.items.traverse_(item => IO.println(s"     ${item.index}. ${item.content}"))
      ...
      _       <- result4.items.traverse_(item => IO.println(s"     ${item.index}. ${item.content}"))
