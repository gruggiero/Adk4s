...
object ToolCallOnceExample extends IOApp.Simple:
  ...
  private def buildGraph(
      ...
      WIONode.pure[Ctx.Ctx, InputState, ConversationState]((input: InputState) =>
        ConversationState(
          Conversation(
            Seq(
              SystemMessage(
                "You are a real estate agent. Use the user_info API to look up user details. Email is required."
              ),
    ...
    val chatNode: WIORunIONode[Ctx.Ctx, ConversationState, Nothing, ChatCompleted, CompletionState] =
      WIONode.runIO[Ctx.Ctx, ConversationState, ChatCompleted, CompletionState](
        ...
        handleEvent = (state: ConversationState, evt: ChatCompleted) =>
          CompletionState(
            evt.completion,
            state.conversation.messages.collect { case msg: UserMessage => msg.content }.lastOption.getOrElse("")
          )
      )
    ...
    val hasToolCall: CompletionState => Boolean = (state: CompletionState) => state.query.contains("@")

    val toolBranchWio: WIO[CompletionState, Nothing, GraphState, Ctx.Ctx] =
      WIONode
        .runIO[Ctx.Ctx, CompletionState, ToolExecuted, ToolResultState](
          ...
          ,
          handleEvent = (state: CompletionState, evt: ToolExecuted) => ToolResultState(state.query, evt.result)
        )
        .toWIO

    val endBranchWio: WIO[CompletionState, Nothing, GraphState, Ctx.Ctx] =
      WIONode
        .pure[Ctx.Ctx, CompletionState, OutputState]((state: CompletionState) =>
