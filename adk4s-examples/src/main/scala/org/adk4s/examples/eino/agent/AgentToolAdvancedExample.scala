  ...
  private def createCalculatorTool: IO[AgentTool] =
    val calculatorAgent: Agent = new Agent:
      ...
      def generate(messages: List[Message], maxSteps: Int): IO[AssistantMessage] =
        ...
        val result: String =
          if request.contains("add") || request.contains("+") then "Result: 42 (addition performed)"
          else if request.contains("multiply") || request.contains("*") then "Result: 84 (multiplication performed)"
          else "Unable to parse calculation"
  ...
  private def createSummarizerTool: IO[AgentTool] =
    val summarizerAgent: Agent = new Agent:
      ...
      def generate(messages: List[Message], maxSteps: Int): IO[AssistantMessage] =
        ...
        val summary: String =
          if text.length > 100 then s"Summary: ${text.take(50)}... (${text.split("\\s+").length} words total)"
          else s"Summary: $text (already concise)"
