...
object ChatTemplateExample extends IOApp.Simple:
  ...
  def run: IO[Unit] =
    for
      ...
      template1: ChatTemplate[IO, Map[String, String]] = ChatTemplate.simple[IO](
        List(
          SystemMessage("You are a {role}. Respond in character."),
          UserMessage("{question}")
        )
      )
      conv1 <- template1.formatConversation(
        Map(
          "role"     -> "pirate",
      ...
      _ <- conv1.messages.toList.traverse_((msg: Llm4sMessage) => IO.println(s"   [${msg.role}]: ${msg.content}"))
      ...
      catTemplate: ChatTemplate[IO, Map[String, String]] = ChatTemplate.simple[IO](
        List(
          SystemMessage("You are a cat. Always respond with cat-like behavior."),
          UserMessage("{question}")
        )
      )
      dogTemplate: ChatTemplate[IO, Map[String, String]] = ChatTemplate.simple[IO](
        List(
      ...
      _ <- conv3.messages.toList.traverse_((msg: Llm4sMessage) => IO.println(s"   [${msg.role}]: ${msg.content}"))
