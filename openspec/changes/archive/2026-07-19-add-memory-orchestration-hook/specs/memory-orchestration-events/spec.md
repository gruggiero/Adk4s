# Spec: Memory Orchestration Events

<!-- This is a DELTA spec. It extends the sealed `AgentEvent` ADT in
     `adk4s-core` with two observability variants. It is SMALL and may be
     deferred independently of the hook spec — the hook spec is
     observability-neutral without it.
     ...
     - This spec EXTENDS a sealed ADT — the exhaustiveness impact section below
       states how every existing pattern match over `AgentEvent` must behave for
