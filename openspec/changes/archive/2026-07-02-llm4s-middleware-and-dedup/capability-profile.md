...
| Formal verification | Stainless (bundled jar + local Maven repo) | Ring 6 available via `verified` module (Scala 3.7.2, `stainlessEnabled := false` by default). Run with `sbt -J-Xmx6g ring6`. |
...
| 6 Formal | ✅ | Stainless via `verified` module (Scala 3.7.2). Run `sbt -J-Xmx6g ring6`. |
