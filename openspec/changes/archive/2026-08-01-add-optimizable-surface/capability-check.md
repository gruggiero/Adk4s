...
| Formal verification note | "NOT applicable to this change (the hook is effectful `IO` wiring...)" | SUPERSEDED by schema v10: applicability is by algorithmic purity; see the R6 row below. Generalized to "Applicable only to PureScala modules in `verified`; effectful `IO` wiring is NOT a fit." | stale annotation pruned |
...
| R6 Formal | ✅ APPLIES (verified-mirror) | Applicability is decided by ALGORITHMIC purity (schema v10), not by whether the shipped code is itself PureScala. `Optimizable`'s `Mirror`/`inline`/`ujson.Value` types and the 3.8.4-vs-3.7.2 frontend pin are exactly why the mirror exists. **Frontend**: Scala 3.7.2. **Mirror module**: existing `verified/` leaf (StainlessPlugin, `stainlessEnabled := false`, alias `sbt -J-Xmx6g ring6`) — gains `PredictorKernel`. **Bridge precondition**: this change adds `adk4s-optimize dependsOn(verified % Test)`, which is what makes `PredictorModelBridgeSpec` possible (previously no module took `verified` as a Test dep, so no bridge could exist). |
