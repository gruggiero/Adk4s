...
| LLM client | llm4s core | 0.3.4 (Maven Central) | LLMClient, Conversation, Message, ToolFunction, ToolRegistry, Result[A], StreamingAccumulator, AgentEvent, Guardrail, RetryPolicy, ReliableClient, LLMMiddleware |
...
| Test count | ~568 tests total (14 + 26 + 158 + 370) | Across structured-llm, structured-llm-test-models, adk4s-orchestration, adk4s-core |
...
| WartRemover | `Warts.unsafe` minus excluded set (see right) | Permanently excluded: `TripleQuestionMark` (intentional stub allowance), `Any` (Scala 3 string interpolation false positive — `StringContext.s` takes `Any*`, 919 sites), `DefaultArguments` (valid Scala API design feature, 47 sites in config case classes). Previously excluded but now re-enabled: `IterableOps`, `AsInstanceOf`, `Throw`, `Var`, `OptionPartial`, `StringPlusAny`. `verified` module: `wartremoverErrors := Seq.empty` (exempt). `adk4s-examples`: same excluded set as ThisBuild. | build.sbt |
...
- For this change: typed contracts go in `structured-llm/src/test/scala/org/adk4s/structured/typecontract/` (most specs) and `adk4s-core/src/test/scala/org/adk4s/core/typecontract/` (fallback/round-robin spec)
