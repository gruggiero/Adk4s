...
| Root aggregate | NO explicit `root` aggregate project in build.sbt; modules are standalone `lazy val`s. `sbt compile` compiles all modules that sbt discovers via the project matrix. | build.sbt |

## Libraries

| Concern | Detected Library | Version | Notes |
|---------|-----------------|---------|-------|
| Effect system | cats-effect | 3.7.0 | All modules are effectful via `F[_]` / `IO`. Brings `cats-core` transitively — no standalone `cats-core` dependency exists. |
| Actors | none | — | No Pekko/Akka |
| HTTP | none | — | No http4s/tapir |
| Persistence | none | — | No Doobie/Skunk/DynamoDB |
| Messaging | none | — | No Kafka |
| Streaming | fs2 (core + io) | 3.13.0 | Used in adk4s-core, adk4s-orchestration, adk4s-examples. `Retriever.retrieveStream` returns `fs2.Stream`. |
| JSON | upickle / ujson | (transitive via llm4s) | Used directly in adk4s-core for tool JSON. `Document.metadata` is `Map[String, ujson.Value]`. NOT circe. |
...
| Mutation tool | sbt-stryker4s 0.21.0 + stryker4s.conf | Ring 5 available. stryker4s.conf has a fixed `mutate` list (`**/sap/SchemaAlignedParser.scala`) — MUST retarget to each spec's changed files before running. Thresholds: break=90, low=91, high=95. |
| Formal verification | Stainless (bundled jar `project/lib/sbt-stainless.jar` + local Maven repo) | Ring 6 available via `verified` module (Scala 3.7.2, `stainlessEnabled := false` by default). Run with `sbt -J-Xmx6g ring6`. |
| Model checking | none | No TLA+/Apalache. Ring 7 skip. |
| Test count | ~551 tests total | Across structured-llm, structured-llm-test-models, adk4s-orchestration, adk4s-core |

## Static Analysis

| Tool | Active Rules | Inactive/Excluded | Evidence |
|------|-------------|--------------------|----------|
| Scalafix | `DisableSyntax` (noVars, noThrows, noNulls, noReturns, noWhileLoops, noAsInstanceOf, noIsInstanceOf, noFinalize + custom regex: NoConfigFactory, NoSysEnv, NoSystemGetenv, NoKeywordTry/Catch/Finally), `RemoveUnused` (imports, privates, locals, patternvars), `OrganizeImports` (Merge, grouped) | Scoped guards for adk4s-core and structured-llm main sources (NoAdk4sConfig, NoPureConfigDefault, NoEnvReads) — aspirational (PureConfig not yet a dependency). `scalafixOnCompile := false` (not enforced on compile; run via `sbt scalafixAll`). | .scalafix.conf |
...
| scalafmt | Config present: scala3 dialect, maxColumn=120, align.preset=more, rewrite.rules=[AvoidInfix, RedundantBraces, RedundantParens, SortModifiers, PreferCurlyFors], trailingCommas=preserve | — | .scalafmt.conf |
...
- For this change: typed contracts for `specs/agent-memory` and `specs/memory-retriever-bridge` live in `adk4s-memory-api/src/test/scala/org/adk4s/memory/typecontract/`. Typed contracts for `specs/memory-testkit` live in `adk4s-memory-testkit/src/test/scala/org/adk4s/memory/testkit/typecontract/`.
...
| `org.adk4s.memory` (NEW — this change, capability interface) | workflows4s, llm4s LLM client, logback, http, smithy4s, typesafe-config | cats-core (typeclasses: `Monad`, `Traverse`, `Functor`), `java.time.Instant`, `org.adk4s.core.component.{Retriever, Document, RetrieverConfig}` (bridge only) |
| `org.adk4s.memory` (NEW — `InMemoryAgentMemory` test double) | workflows4s, llm4s, logback, http, smithy4s | cats-effect (`Ref`, `Sync`), fs2 (for `MemoryRetriever.retrieveStream`), cats-core, `java.time.Instant` |
| `org.adk4s.memory.testkit` (NEW — `AgentMemoryLaws`) | workflows4s, llm4s, logback, http, smithy4s | cats-effect (`IO`), munit assertions, `org.adk4s.memory.*` |
...
|------|-----------|--------------------------------------|
| 0 Compile | ✅ | `sbt compile` — all 6 modules. New modules: `sbt adk4s-memory-api/compile`, `sbt adk4s-memory-testkit/compile`. |
| 1 Lint | ✅ | Scalafix (DisableSyntax + RemoveUnused + OrganizeImports) + WartRemover (relaxed set) + scalafmt. New modules inherit `ThisBuild` wartremoverErrors. |
| 2 Architecture | ⚠️ Advisory only | No custom scalafix arch rules installed. Layer rules in table above are manual. For this change: verify `adk4s-memory-api` has no heavy deps via `sbt "adk4s-memory-api/dependencyTree"`. |
| 3 Property tests | ✅ | Hedgehog 0.13.1 via hedgehog-munit. Properties extend `HedgehogSuite`. munit-cats-effect for `IO`-based scenario tests. |
| 4 Compatibility | ⚠️ Manual | No fixture-based compatibility framework. N/A for this change (no serialization touched). |
| 5 Mutation | ✅ | sbt-stryker4s 0.21.0 + stryker4s.conf. Retarget `mutate` list to `**/memory/InMemoryAgentMemory.scala` and `**/memory/MemoryRetriever.scala` for this change. |
| 6 Formal | ✅ | Stainless via `verified` module (Scala 3.7.2). Not applied for this change — `naiveScore` is too thin to justify a Stainless mirror; Hedgehog properties cover the same ground. |
| 7 Model checking | ❌ | No TLA+/Apalache. Skip with stated correctness impact. |
| 8 Adversarial review | ✅ (manual — always available) | Performed in `spec-lint` artifact. |
| 9 Telemetry | ❌ | No otel4s/Daut. Skip — no `AgentEvent` variants added in this change. |

## Change-Specific Notes (add-memory-api)

<!-- What this change adds to the detected stack. These are NEW capabilities
     introduced by the change itself, not pre-existing. -->

| Item | Value | Notes |
|------|-------|-------|
| New module: `adk4s-memory-api` | `dependsOn(adk4s-core)`, main deps `cats-effect` + `fs2-core` (for `Ref`/`Sync` in `InMemoryAgentMemory` and `Stream` in `MemoryRetriever.retrieveStream`), test deps `Dependencies.testDeps` | Effect-polymorphic interface (`AgentMemory[F]`); `Sync[F]` only on implementations/bridge. |
| New module: `adk4s-memory-testkit` | `dependsOn(adk4s-memory-api)`, main deps `cats-effect` + `munit` (NOT test-scoped — `AgentMemoryLaws` is a downstream-consumable main API) | Publishes `AgentMemoryLaws` so GraphStore can depend on it as a regular `libraryDependencies` line. |
| New package: `org.adk4s.memory` | lives in `adk4s-memory-api/src/main/scala/org/adk4s/memory/` | `Episode`, `SourceType`, `EpisodeOutcome`, `MemoryHit`, `TemporalScope`, `AgentMemory`, `MemoryRetriever`, `InMemoryAgentMemory`. |
| New package: `org.adk4s.memory.testkit` | lives in `adk4s-memory-testkit/src/main/scala/org/adk4s/memory/testkit/` | `AgentMemoryLaws`. |
| Build wiring | No `root` aggregate exists; new modules are standalone `lazy val`s like the existing ones. A build-wiring task decides whether to introduce a root aggregate or document per-module invocation. | Matches current convention (`sbt "adk4s-memory-api/compile"`). |
| Reused types | `org.adk4s.core.component.{Retriever, Document, RetrieverConfig}` | Verified shapes: `Retriever[F]` is a trait with `retrieve(query, config)` + `retrieveStream(query, config)`; `Document(id, content, metadata: Map[String, ujson.Value])` — no `score` field. |
| `cats-core` sourcing | Transitive via `cats-effect` (no standalone `cats-core` dep in `Dependencies.scala`) | The doc's "main scope pulls only `cats-core`" is not realizable here; `cats-effect` is the main-scope dep. |
