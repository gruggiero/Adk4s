# json-value-model Specification

<!-- The `JsonValue` internal type, the llm4s boundary adapter, and the
     architectural rule that `ujson.Value` may only appear in code that
     directly interoperates with `org.llm4s.toolapi`.

     This is the foundation spec for the migrate-json-codec change. The
     other 8 specs in that change depend on `JsonValue` existing and on the
     boundary adapter being the single conversion point. -->

## Purpose
Provides ADK4S's own internal, immutable JSON value type (`JsonValue`, a type alias over `smithy4s.Document`) and the single boundary adapter (`JsonValueCodec`) that converts to/from `ujson.Value` at the llm4s edge.
