# memory-orchestration-hook Specification

## Purpose
Make an agent memory-aware by decorating an AgentRunner with a pre-turn recall (injects relevant facts into the prompt) and a post-turn remember (persists user input and/or assistant output as episodes), skipping the write on Interrupted/Failed. Opt-in: when no memory is supplied, the decorator is the identity.
