# cross-run-memory-example Specification

## Purpose

Runnable examples demonstrating cross-process durable memory via the `AgentMemory` contract: a file-backed `AgentMemory` double (`FileBackedAgentMemory`) that persists `Episode`s as JSON lines, a CLI-driven `CrossRunMemoryExample` showing teach/recall across separate invocations through `MemoryAwareRunner`, and a `MemoryRetrieverExample` showing the `MemoryRetriever` → `Retriever` seam. Created by archiving change `add-cross-run-memory-example`.
...
| Retriever returns top-k with metadata | Requirement: Memory retriever example shows the second seam + Scenario: Retriever returns top-k documents | manual review (documented manual run — no automated suite) | `MemoryRetrieverExample.scala` |
| Retriever with no matches returns empty | Requirement: Memory retriever example shows the second seam + Scenario: Retriever with no matching episodes (edge path) | manual review (documented manual run — no automated suite) | `MemoryRetrieverExample.scala` |
